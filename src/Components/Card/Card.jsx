         ...
         <div className={styles['header']}>
            ...
            <div style={{ position: 'absolute', right: 0 }}>
               {display.grouping !== "userId" && (
                  <UserCircle name={ticket.user.name} status={ticket.user.available ? "active" : "inactive"} />
               )}
            </div>
         </div>
...
export const MemoizedTicketCard = ({ ticket, display }) => {
   return useMemo(() => <TicketCardComponent display={display} key={ticket.id} id={ticket.id} ticket={ticket} />, [
      ticket,
      display,
   ]);
};
